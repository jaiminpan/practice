...
# convert sogou input dict ( .scel file ) to 
# mmseg(coreseek) dict or jieba dict
...
def store_mmseg(records, f):
    for (pystr, utf8str) in records:
        f.write("%s\t1\n" %(utf8str.encode("utf8")))
        f.write("x:1\n")

def store_jieba(records, f):
    for (pystr, utf8str) in records:
        f.write("%s\n" %(utf8str.encode("utf8")))

def store(records, f):
    store_jieba(records, f)

def usage(procname):
    print "Unknown Option \n usage: python %s file.scel new.txt" % procname
